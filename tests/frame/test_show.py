...
def test_produce_window(use_output: bool) -> None:
    ...
    produce_window(input_queue, output_queue, key_queue, frame_pool)

    assert input_queue.empty()
    assert not frame_pool.has_free_slots()

    clear_queue(input_queue, frame_pool)
    clear_queue(key_queue)
    if output_queue:
        clear_queue(output_queue, frame_pool)
    else:
        for frame in frames:
...
def test_window_producer(
    ...
    clear_queue(clean_queue, frame_pool)
    clear_queue(key_queue)
...
def test_window_producer_logs(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.INFO):
        produce_window(input_queue, None, key_queue, None, 2)
